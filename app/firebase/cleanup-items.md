...
- [ ] The coordinated workflow now targets the seven exact LaundryOps Functions and never uses a broad Functions deployment. `syncAmazonIncomeSheet` remains excluded. Keep pending until the targeted deployment and live Function verification succeed.
- [ ] Server-side Firestore transaction rate limits are implemented and tested for checkout, billing portal, manual indexing, bulk reindexing, manual deletion, and Repair Assist. Keep pending until the targeted deployment proves live enforcement; Stripe webhook retries remain protected by signature verification instead of user rate limiting.
